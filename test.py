...
def main(args):
    ...
    # Build test dataset with preprocessing config (MNI or canonical based on config)
    ds_test = MRIVolumeDataset(
    ...
    with torch.no_grad():
        for x, y, p in dl_test:
            # x is already preprocessed and shaped [1, 1, D, H, W]
            x = x.to(device, non_blocking=True).float()

            logits = model(x)
            prob_ad = torch.softmax(logits, dim=1)[:, 1].cpu().item()
            pred = int(prob_ad >= th)
            ...
            # Use the preprocessed tensor for visualization (already target_shape)
            vol = x[0, 0].cpu().numpy()  # [D, H, W] float32
            ...
            if cfg["test"].get("save_slices", True):
                ...
                out_img = os.path.join(
                    slices_dir,
                    os.path.basename(p[0]).replace(".nii.gz", "_slices.png").replace(".nii", "_slices.png")
                )
                save_three_orthogonal_slices(
                    vol, out_img, title=title, cmap=cfg["test"].get("cmap", "gray"), idx=idx
                )
            ...
            preds_for_grid.append(pred)
            labels_for_grid.append(int(y.item()))
    ...
    with open(os.path.join(out_dir, "test_predictions.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["path", "true", "pred", "prob_ad"])
        for pth, yt, yp, pr in zip(paths, y_true, y_pred, y_prob):
            w.writerow([pth, yt, yp, f"{pr:.6f}"])

    grid_cols = 5
    save_slice_grid(
        axial_slices, preds_for_grid, labels_for_grid,
        os.path.join(slices_dir, "grid_axial.png"),
        cols=grid_cols, cmap=cfg["test"].get("cmap", "gray"), title="Axial Slices"
    )
    save_slice_grid(
        coronal_slices, preds_for_grid, labels_for_grid,
        os.path.join(slices_dir, "grid_coronal.png"),
        cols=grid_cols, cmap=cfg["test"].get("cmap", "gray"), title="Coronal Slices"
    )
    save_slice_grid(
        sagittal_slices, preds_for_grid, labels_for_grid,
        os.path.join(slices_dir, "grid_sagittal.png"),
        cols=grid_cols, cmap=cfg["test"].get("cmap", "gray"), title="Sagittal Slices"
    )
